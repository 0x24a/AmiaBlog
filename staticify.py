...
class AmiaBlogStaticGenerator:
    ...
    def load_data(self):
        ...
        self.hljs_manager = HLJSLanguageManager(self.config.site_settings.hljs_languages)
        self.posts_manager = PostsManager(search_method="fullmatch", build_search_index=False)
        ...
        self.renderer = TemplateRenderer(
            ...
            static_params={
                "config": self.config,
                "i18n": self.i18n,
                "backend_version": __VERSION__ if __VERSION__ else "unknown" + "-static",
    ...
    def render_top_layers(self):
        ...
        self.renderer.render_static(
            ...
            "posts.html",
            posts=self.posts_manager.order_by(list(self.posts_manager.posts.values()), "modified_desc"),
        )
        logger.info("\tRendering: tags.html")
        tags = self.posts_manager.list_tags(order_by="post_count")
        self.renderer.render_static(
            os.path.join(self.destination, "tags.html"), "tags.html", tags=tags, n_tags=len(tags)
        )
        logger.info("\tRendering: 404.html (need extra configuration to work)")
        self.renderer.render_static(
            os.path.join(self.destination, "404.html"), "error.html", error=self.i18n.page_not_found
        )
    ...
    def render_posts(self):
        if not self.posts_manager.posts:
            ...
            for slug, post in self.posts_manager.posts.items():
                ...
                available_languages = [
                    i for i in hljs_languages if i in self.hljs_manager.available_languages
                ]
                with open(os.path.join(self.destination, f"post/{slug}.html"), "w+") as f:
                    f.write(
    ...
    def render_tags(self):
        ...
        if not tags:
            ...
            for tag in tags:
                ...
                with open(os.path.join(self.destination, f"tag/{tag.name}.html"), "w+") as f:
                    f.write(
                        self.renderer.render_to_plain_text("tag.html", tag=tag.name, posts=posts)
                    )
    ...
    def render(self):
        ...
        logger.info(f"Generating static site with AmiaBlog v{__VERSION__} to {self.destination}")
        logger.info("Loading site data")
        self.load_data()
        logger.info("Site data loaded successfully.")
        self.renderer.static_params.update(
            {
                "static_build_time": build_time
            }
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--destination", help="Destination directory", default="dist", required=False)
    parser.add_argument("--remove-existing", action="store_true", help="Overwrite existing destination directory", default=False, required=False)
